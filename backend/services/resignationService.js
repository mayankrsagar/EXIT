...
//         text: `Employee ${employee.username} (ID: ${employee._id}) has submitted a resignation. LWD: ${intendedLastWorkingDay}`,
//       });
//     } catch (mailerErr) {
...
export const submitResignation = async ({ employeeId, intendedLastWorkingDay }) => {
  ...
  try {
    ...
    console.error(`Calendarific error: ${calErr.message}`);
 ...
 *   - approved: boolean
 *   - lwd: string (exit date if approved)
 */
export const concludeResignation = async ({ resignationId, approved, lwd }) => {
